...
fn main() {
    ...
    let mut count = 0;
    for item in &data {
        match regex.captures(item) {
            Some(x) => {
                let min = x.get(1).unwrap().as_str().parse::<i32>().unwrap();
                let max = x.get(2).unwrap().as_str().parse::<i32>().unwrap();
                let c = x.get(3).unwrap().as_str();
                let pwd = x.get(4).unwrap().as_str();
                let nb = pwd.matches(c).count() as i32;
                if (min <= nb) && (nb <= max) {
                    count += 1;
                    //println!("Valid password: {}", item);
                }
            }
            None => unreachable!(),
        }
    }
    println!("Number of valid passwords (method 1): {}", count);

    let mut count = 0;
    for item in &data {
        match regex.captures(item) {
            Some(x) => {
                let min = x.get(1).unwrap().as_str().parse::<usize>().unwrap();
                let max = x.get(2).unwrap().as_str().parse::<usize>().unwrap();
                let c = x.get(3).unwrap().as_str().chars().nth(0).unwrap();
                let pwd = x.get(4).unwrap().as_str();
                if (pwd.chars().nth(min - 1).unwrap() == c)
                    ^ (pwd.chars().nth(max - 1).unwrap() == c)
                {
                    count += 1;
                    //println!("Valid password: {}", item);
                }
    ...
    println!("Number of valid passwords (method 2): {}", count);
}
