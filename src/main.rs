...
fn main() {
    let data = "striped orange bags contain 1 vibrant green bag, 5 plaid yellow bags, 1 drab magenta bag.
...
mirrored orange bags contain 2 dim green bags, 2 striped red bags, 4 drab plum bags, 1 pale teal bag.";
    let _test = "light red bags contain 1 bright white bag, 2 muted yellow bags.
...
dotted black bags contain no other bags.";
    let _test2 = "shiny gold bags contain 2 dark red bags.
...
dark violet bags contain no other bags.";
    ...
    let _test2: Vec<_> = _test2.split("\n").collect();
    let rules = parse_rules(_data);

    //println!("{:#?}", rules);

    let find = "shiny gold";
    let result = search_in_rules(&rules, find);
    println!(
        "The rules that \"{}\" match are {}: {:?}",
        find,
        result.len(),
        result
    );
    ...
    println!(
        "The number of individual bags for \"{}\" are {}",
        find,
        traverse_rules(&rules, find) - 1 //All but itself
    );
}

fn parse_rules(data: Vec<&str>) -> HashMap<&str, Vec<(u8, String)>> {
    let mut rules: HashMap<&str, Vec<(u8, String)>> = HashMap::new();
    for item in &data {
        match Regex::new(r"^(.*) bags contain (.*)$")
            .unwrap()
            .captures(&item)
        {
            Some(m) => {
                let key = m.get(1).unwrap().as_str();
                let values: String = m.get(2).unwrap().as_str().to_string();
                let tuples: Vec<String> = values.split(", ").map(|s| s.to_string()).collect();
                let mut x = Vec::new();
                for value in tuples {
                    match Regex::new(r"^(\d*) (.*) bag").unwrap().captures(&value) {
                        Some(n) => {
                            x.push((
                                n.get(1)
                                    .unwrap()
                                    ...
                                    .parse::<u8>()
                                    .unwrap(),
                                n.get(2).unwrap().as_str().to_string(),
                            ));
                        }
                        None => {
                            if let None = Regex::new(r"^no other bags").unwrap().captures(&value) {
                                println!("{:?}", value);
                                unreachable!()
                            }
                        }
...
fn search_in_rules<'a>(
    rules: &HashMap<&'a str, Vec<(u8, String)>>,
    find: &'a str,
) -> HashSet<&'a str> {
    let mut matches: HashSet<&str> = HashSet::new();
    for (key, vec) in rules {
        if vec.iter().filter(|x| x.1 == find).count() > 0 {
            matches.insert(key);
        }
    }
    let mut others: HashSet<&str> = HashSet::new();
    for item in &matches {
        others.extend(search_in_rules(rules, item));
    }
    matches.extend(others);
    matches
}

fn traverse_rules<'a>(rules: &HashMap<&'a str, Vec<(u8, String)>>, find: &'a str) -> u32 {
    let mut count: u32 = 1;
    for item in &rules[find] {
        for _ in 0..item.0 {
            count += traverse_rules(rules, &item.1);
        }
    }
    count
}
